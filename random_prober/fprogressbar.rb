...
class FastProgressBar
  ...
  def bar
    len = percentage * @bar_length / 100.0
  ...
  def show_progress
    if @total.zero?
      cur_percentage = 100.0
      prev_percentage = 0.0
    else
      cur_percentage  = (@current  * 100.0 / @total).to_i
      prev_percentage = (@previous * 100.0 / @total).to_i
    end
