...
static int wc_queue_extract(struct wc_queue *pq , int *, struct timeval * key, void (**fun)(void *), void **arg);
...
wc_queue * wc_queue_init(int initSize){
	...
	pq = malloc_and_test(sizeof(wc_queue));
	if(pq==NULL){
		perror("wc_queue_init:: malloc");
		return NULL;
	}
	pq->array = malloc_and_test(sizeof(qelm)*initSize);
	if(pq->array == NULL){
		perror("wc_queue_init:: malloc");
        free(pq);
		return NULL;
...
int wc_event_add(wc_queue* pq, void (*fun)(void *), void *arg, struct timeval key){
	...
	pq->size++;
	i=pq->size;
	data = malloc_and_test(sizeof(wc_event));
    assert(data != NULL);
	data->fun=fun;
	data->eventtime=key;
	data->arg=arg;
...
static void wc_schedule_next_event(wc_queue *pq)
{
	...
	while(!wc_queue_isempty(pq))		
	{
		...
		{	// event already should have happened; sched for NOW
			sidecarlog(LOGDEBUG,"Scheduling event %d NOW b/c it has passed already\n",data->id);
...
int wc_run_next_event(wc_queue * pq)
{
	...
	if(diff.tv_sec<1)			// that this event went off less then 1 sec after it was 
		...
		logtype=LOGCRIT;
						// intended to
	sidecarlog(logtype," issuing timer %d for %ld.%.6ld (%ld.%.6ld late)\n",
...
void wc_queue_double(wc_queue *pq){
	...
	neoarray = malloc_and_test(sizeof(qelm)*pq->length*2);
	assert(neoarray);
...
void wc_disable_timers(struct wc_queue *pq)
{
	...
	sidecarlog(LOGDEBUG," timers disabled\n");
	pq->timersEnabled=0;
}
...
{
	pq->timersEnabled=1;
	sidecarlog(LOGDEBUG," timers enabled\n");
	if(!wc_queue_isempty(pq))
